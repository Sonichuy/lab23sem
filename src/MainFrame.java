...
public class MainFrame extends JFrame {
    ...
    public Double calculate1(Double x, Double y, Double z) {
        double lnz = Math.log1p(z);
        double ex = exp(x);
        double xi = 1/x;
        return (sqrt(sqrt(pow(lnz, 2) + cos(PI*pow(y, 3)))))/(pow(cos(ex) + sqrt(xi) + pow(ex, 2), sin(x)));
    }

    public Double calculate2(Double x, Double y, Double z) {
        double zy = pow(z, y);
        return (pow(sin(zy), 2))/(sqrt(1 + pow(x, 3)));
    }
    ...
    public MainFrame() {
        ...
        hboxVariables.setBorder(BorderFactory.createLineBorder(Color.GREEN));
        ...
        memoryPanel.setBorder(BorderFactory.createLineBorder(Color.CYAN));
